...
DEFAULT_SYSTEM_PROMPTS = {
    ...
    "json_ld": (
        ...
        "You will be given raw JSON-LD data extracted from a webpage. "
        "Map and clean these fields into the target schema with high precision. "
        "\n\n### FIELD GUIDANCE:\n"
        "- 'is_job_post': Always set to True for incoming JobPosting data.\n"
        "- 'likelihood': Set to 1.0.\n"
        "- 'role': Use the JSON 'title' field. Extract the job title verbatim.\n"
        "- 'company': Look for 'hiringOrganization.name', 'hiringOrganization' as a string, or 'brand.name'.\n"
        "- 'location': Look for 'jobLocation'. Combine 'addressLocality', 'addressRegion', and 'addressCountry' into \"City, State, Country\".\n"
        "- 'salary_range': Look for 'baseSalary'. Map 'minValue', 'maxValue', and 'currency' into a string like \"$Min - $Max Currency\".\n"
        "- 'company_job_id': Look for 'identifier.value', 'identifier' as a string, or any 'jobID' key.\n"
        "- 'description': This is likely HTML. Convert it to clean, professional **Markdown**. PRESERVE VERBATIM text. Use `###` for headers and `-` for lists to prettify the output. Do NOT invent new headers.\n"
        "- 'job_posted_date': Convert 'datePosted' to YYYY-MM-DD.\n"
        "- 'application_deadline': Use 'validThrough' or 'expires'. Convert to YYYY-MM-DD.\n"
        "- 'detected_category': Set to 'Job Post'.\n"
