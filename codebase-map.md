# 🗺️ Lard - Lazy AI-Powered Resume Database (v0.56.14)
Last Updated: 2026-04-11T01:19:00Z
...
- **AI Validation & Completeness**: Uses a LangGraph-based `extraction_validator_node` that runs a 3-retry loop to ensure extracted descriptions are clean Markdown and complete, while also performing heuristic validation on metadata fields (Company, Role, etc.) to trigger source fallbacks if necessary. Robustly determines source type (JSON-LD vs. TEXT) based on available data to prevent validation errors on PDFs or non-structured URLs.
