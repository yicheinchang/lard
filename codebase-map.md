...
- **AI Validation & Completeness**: Uses:
    ...
    - **text_validator_node**: Specialized validator for raw text data. Performs semantic boundary detection to isolate the job description from page noise and checks for hallucinations. Includes a **Fast Pass** mechanism: if a description was already verified by the JSON fidelity pass, it bypasses the text boundary check to prevent false-positive rejections on noisy pages.
